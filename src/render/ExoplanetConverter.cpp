...
namespace astrocore
{
    ...
    std::string ExoplanetConverter::inferPlanetType(const ExoplanetData &exo)
    {
        ...
        if (density > 0)
        {
            if (density < 2.0)
            {
                ...
                if (massEarth > 50 || radiusEarth > 6.0)
                    return "Gas Giant";
                return "Neptune-like";
            }
            if (density >= 2.0 && density < 4.0)
            {
                ...
                if (massEarth > 5 || radiusEarth > 2.5)
                    return "Neptune-like";
                if (tempK > 0 && tempK < 350)
                    return "Ocean World";
                return "Super-Earth";
            }
            ...
            if (tempK > 1500)
                return "Lava World";
            if (massEarth > 1.5 || radiusEarth > 1.25)
                return "Super-Earth";
            return "Terrestrial";
        }
        ...
        if (massEarth > 0 && radiusEarth == 0)
        {
            ...
            {
                if (stellarMet > 0.1)
                    return "Neptune-like"; // Metal-rich star
        ...
        if (radiusEarth > 0)
        {
            ...
            if (radiusEarth > 3.5)
                return "Neptune-like";
            if (radiusEarth > 2.0)
                return "Neptune-like"; // Sub-Neptune
            if (radiusEarth > 1.6)
            {
                // In the transition zone - temperature matters
                if (tempK > 1000)
                    return "Super-Earth"; // Lost atmosphere
    ...
    glm::vec3 ExoplanetConverter::parseColorHint(const std::string &hint)
    {
        ...
        if (lower.find("red") != std::string::npos)
            return {0.8f, 0.3f, 0.2f};
        if (lower.find("orange") != std::string::npos)
            return {0.9f, 0.5f, 0.2f};
        if (lower.find("yellow") != std::string::npos)
            return {0.9f, 0.85f, 0.4f};
        if (lower.find("green") != std::string::npos)
            return {0.3f, 0.7f, 0.3f};
        if (lower.find("blue") != std::string::npos)
            return {0.3f, 0.5f, 0.9f};
        if (lower.find("purple") != std::string::npos)
            return {0.6f, 0.3f, 0.8f};
        if (lower.find("brown") != std::string::npos)
            return {0.6f, 0.4f, 0.2f};
        if (lower.find("tan") != std::string::npos)
            return {0.8f, 0.7f, 0.5f};
        if (lower.find("white") != std::string::npos)
    ...
    {
        // Build cache path: ~/.cache/astrodex/planets/{sanitized_name}.json
